...
func TestMsgStore(t *testing.T) {
	...
	// I can add a message
	err := s.Set(msg.New("foo", "bar"))
	require.NoError(t, err)
	assert.Equal(t, 1, s.Len())

	// I can add a message with plurals
	err = s.Set(msg.New("foos.zero", "There are no foos."))
	require.NoError(t, err)
	assert.Equal(t, 2, s.Len())

	// I can add the remaining plurals
	err = s.Set(msg.New("foos.one", "There is one foo."))
	require.NoError(t, err)
	assert.Equal(t, 2, s.Len())
	err = s.Set(msg.New("foos.many", "There are _ foos."))
	require.NoError(t, err)
	assert.Equal(t, 2, s.Len())

	// I can add another message
	err = s.Set(msg.New("bar", "There is a bar."))
	require.NoError(t, err)
